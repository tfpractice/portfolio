const coercion = {
  caption: `### iterability and coercion`,
  content: `This is one of the projects I've been most proud of, and marks the moment in my development experience at which my interests moved from programming solutions to programming about programming. Having developed a certain comfort with the complicated things I could do _with_ code, I became interested in what the code _itself_ could do. And as I was moving toward a more collection-based functional style, I decided to explore Arrays, Sets, and Maps.

The conceptual root of this entire project is iterability, and in javascript all collections have the \`[Symbol.iterator]\` property. This led to two functions which form the basis of the project. \`isIterable\`, which checks if an object can contain acts as collection and \`iterify\`, which coerces an object into an iterable if it is not
...
const rewrite = {
  caption: `### rewiriting the basics`,
  content: `
Once I had successfully coerced things to the structure I wanted,abstracting native methods such as \`Array#push\` and \`Array#slice\` became a breeze.
...
const sets = {
  ...
  content: `  
The final part of this project was implementing mathematical set operations. While many external implementations of set methods are key-based (using an id-accessor function/property), my interest in immutability and interoperability suggested that I was only changing the containers, and not the objects themselves. 
